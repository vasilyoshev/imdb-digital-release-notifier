...
export default function Home() {
  ...
  const loadStatus = async () => {
    const s = await fetch("/api/status").then((res) => res.json());
    setStatus(s);
  };
    ...
    <div className="min-h-screen bg-base-200">
      <div className="max-w-4xl mx-auto p-6 space-y-6">
        ...
        <p className="opacity-70">
          Reads your IMDb watchlist ({status?.watchlistUrl || "set WATCHLIST_URL in .env.local"})
          and matches against the DVDsReleaseDates RSS.
        </p>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div className="card bg-base-100 shadow">
            <div className="card-body">
              ...
              <p className="text-sm opacity-70">
                Runs one full cycle (fetch watchlist, parse RSS, email new matches).
              </p>
              <button
                className={`btn btn-primary ${refreshing ? "btn-disabled" : ""}`}
              ...
              <dialog id="refresh_toast" className="modal modal-bottom sm:modal-middle">
                <div className="modal-box">
                  ...
                  <p className="py-4">
                    Watchlist refreshed and feed checked. Emails sent for new matches.
                  </p>
          ...
          <div className="card bg-base-100 shadow">
            <div className="card-body">
              ...
              <p className="text-sm opacity-70">
                Compares current stored titles to the RSS (no email).
              </p>
              <button
                className={`btn btn-secondary ${checking ? "btn-disabled" : ""}`}
                onClick={checkNow}
                disabled={checking}
              >
            ...
            {!status ? (
              ...
              <div className="stats stats-vertical lg:stats-horizontal shadow">
                <div className="stat">
                  ...
                  <div className="stat-value text-sm truncate max-w-xs">
                    {status.watchlistUrl || "—"}
                  </div>
                </div>
                <div className="stat">
        ...
        <div className="card bg-base-100 shadow">
          <div className="card-body">
            ...
            {matches.length === 0 ? (
              <div className="alert">
                <span>No matches yet. Try “Check now”.</span>
              </div>
            ) : (
                  ...
                  <tbody>
                    {matches.map((m, i) => (
                      <tr key={i}>
                        ...
                        <td>
                          <a className="link link-primary" href={m.link} target="_blank">
                            Open
                          </a>
                        </td>
                      </tr>
