...
function Upcoming() {
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {awaiting.length > 0 && (
        <div>
          <div className="mb-1 text-[11px] font-medium uppercase tracking-wide text-base-content/40">
            Awaiting digital <span className="opacity-60">· no date yet</span>
          </div>
          <ul className="flex flex-col">
            {awaiting.map((a) => (
              <li key={a.movieId} className="flex items-baseline gap-2 py-1.5 text-sm">
                <span aria-hidden="true">⏳</span>
                <div className="min-w-0">
                  <span className="font-medium">{a.title}</span>
                  <div className="font-mono text-xs text-base-content/40">
                    in theaters {fmtChip(a.theatricalDate)}
                    {a.region && <sup className="ml-0.5 opacity-60">{a.region}</sup>}
                  </div>
                </div>
              </li>
            ))}
